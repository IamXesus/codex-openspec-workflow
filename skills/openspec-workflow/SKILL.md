...
- A reviewer finding or `READY` verdict never expands the accepted scope. Architecture, ACL/security, persistent-data, transaction, deletion, migration, and external-effect changes require a new or revised decision accepted by the owner before implementation.
- A primary agent MUST NOT block on a subagent. Delegation is optional, bounded, and independent; continue the main path while a helper works and validate its output before integration.
...
- For explicit `skip_specs: true`, the proposal must also contain the structured no-behavior-delta marker required by the shared validator. Do not use it for observable, public-contract, persistent-data, security, transaction, migration, or external-effect changes.
...
- If review or diagnosis proposes a material scope change, record it as proposed and pause before editing; do not turn a reviewer `READY` into approval.
...
6. After a fresh session or context compaction, re-read current authority, active artifacts, open questions, repository root, branch/HEAD/upstream/dirty state, and explicit execution constraints before continuing. Verify completion or process absence before claiming a stopped background command.
...
- Never wait for a subagent as a prerequisite for the primary path. If delegation is used, give it a bounded independent output and continue locally; validate its result before integration.
...
- Use low effort as the default hint for direct small work and medium for evidence-heavy, architecture/security/persistent-data, and release review. Effort is a hint only and never changes route, scope, authority, evidence, review, CI, or GO requirements.
...
- The reviewer also MUST NOT authorize a new architecture, ACL/security, persistent-data, transaction, deletion, migration, rollback, or external-effect strategy. Such a recommendation is advisory until the owner accepts the exact revised scope.
